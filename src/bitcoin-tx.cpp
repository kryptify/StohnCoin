...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " stohn-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  stohn-tx [options] <hex-tx> [commands]  Update hex-encoded stohn transaction\n" +
            "or:     stohn-tx [options] -create [commands]   Create hex-encoded stohn transaction\n" +
            "\n";
